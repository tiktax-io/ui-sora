 ...
 * ```
 * npx jscodeshift -t node_modules/ui-sora/codemods/4.28.1-4.29.0/replace-icon-imports.js --parser=tsx --extensions=js,ts,tsx <your file target> --dry --print
 * ```
 *
 * Converts:
...
export default function transform(file, api) {
  ...
  if (soraImports.length === 0) {
    return
  }

  // Assuming there are no duplicate import declarations
  const soraImport = soraImports[0]
  ...
  root.findJSXElements('Icon')
    ...
    .forEach(({ value: Node }) => {
      ...
      const hasImportAlready = soraImport.specifiers.some(
        s => s.imported.name === componentName
      )
      if (!hasImportAlready) {
        soraImport.specifiers.push(j.importSpecifier(j.identifier(componentName)))
      }
    })
    ...
    for (const prop of props) {
      components
        ...
        .replaceWith(nodePath => {
          ...
          if (!hasImportAlready) {
            soraImport.specifiers.push(j.importSpecifier(j.identifier(iconName)))
          }
